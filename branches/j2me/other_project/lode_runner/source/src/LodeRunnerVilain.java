...
class LodeRunnerVilain extends LodeRunnerCharacter
{
  ...
  {
    super.moveToTile(tile); lookLeft = true; isTrapped = false;
  }
  ...
  public int getFrame()
  {
    ...
    {
      if (delayBusy > DELAY_RESPAWN/2 ) return 76; else return 77;
    }
    // When trapped into a digged hole...
    if (isTrapped && currentMove == MOVE_NONE)
    {
      //... use falling sprites at the beggining ...
      if (delayBusy > DELAY_RESPAWN) return lookLeft ? keyFrames[0] + 5 : keyFrames[0] + 6;
  ...
  {
    if (nChests == 0) return super.takeChest();
    else return false;
  }
  ...
  protected void setCurrentMove(int move)
  {
    ...
    if (move == MOVE_CLIMB_HOLE) { xDelta = 0; yDelta = -1; }
  }
  ...
  protected boolean isPossibleMove(int move)
  {
    ...
    else isPossible = super.isPossibleMove(move);
    if (isPossible)
    {
  ...
  private boolean findAccess(int move)
  {
    ...
    ghost.moveToTile(stage.getTileIndex(xTile, yTile));
    while (ghost.isPossibleMove(MOVE_RUN_LEFT))
    {
      ghost.xTile--; if (ghost.isPossibleMove(move)) { xLeft = xTile - ghost.xTile; break; }
    }
    ghost.moveToTile(stage.getTileIndex(xTile, yTile));
    while (ghost.isPossibleMove(MOVE_RUN_RIGHT))
    {
      ghost.xTile++; if (ghost.isPossibleMove(move)) { xRight = ghost.xTile - xTile; break; }
    }
    ghost = null;
    // No reachable access, don't take any move yet
    if (xLeft == 0 && xRight == 0) return tryNextMove(MOVE_NONE);
    // Only one access, take it
    if (xRight == 0) return tryNextMove(MOVE_RUN_LEFT);
    if (xLeft == 0) return tryNextMove(MOVE_RUN_RIGHT);
    ...
    if (xLeft < xRight) return tryNextMove(MOVE_RUN_LEFT);
    if (xLeft > xRight) return tryNextMove(MOVE_RUN_RIGHT);
  ...
  protected void computeNextMove()
  {
    ...
    if (initialMove != MOVE_CLIMB_HOLE && stage.hero != null)
    {
      ...
      {
        if (stage.hero.yDelta > 0) yHero += 2;
        else if (stage.hero.yDelta < 0) yHero -= 2;
      }
      // Try to move up or down towards the hero
      if (yHero < yTile && tryNextMove(MOVE_CLIMB_UP)) return;
      if (yHero > yTile && tryNextMove(MOVE_CLIMB_DOWN)) return;
      // Try to reach a ladder up or a way down towards the hero
      if (yHero < yTile && findAccess(MOVE_CLIMB_UP)) return;
      if (yHero > yTile && findAccess(MOVE_CLIMB_DOWN)) return;
      ...
      if (yHero == yTile || initialMove == MOVE_NONE)
      {
        ...
        if (stage.hero.xTile < xTile && tryNextMove(MOVE_RUN_LEFT)) return;
        if (stage.hero.xTile > xTile && tryNextMove(MOVE_RUN_RIGHT)) return;
        // Try to move in the same direction as the hero
        if (tryNextMove(stage.hero.lookLeft ? MOVE_RUN_LEFT : MOVE_RUN_RIGHT)) return;
      }
    }
    // Continue initial move
    if (tryNextMove(initialMove)) return;
    // Try a random move (prevent vilains from being stuck)
    switch (stage.random.nextInt(4))
    {
      case 0: tryNextMove(MOVE_CLIMB_UP); break;
      case 1: tryNextMove(MOVE_CLIMB_DOWN); break;
      case 2: tryNextMove(MOVE_RUN_LEFT); break;
      case 3: tryNextMove(MOVE_RUN_RIGHT); break;
    }
  }
  ...
  protected void makeNextMove()
  {
    ...
    if (isTrapped)
    {
      ...
      { delayBusy = DELAY_TRAPPED_HOLE; setCurrentMove(MOVE_NONE); nextMove = MOVE_CLIMB_HOLE; }
      // - he can escape the trap
      else if (nextMove == MOVE_CLIMB_HOLE)
      {
        if (isPossibleMove(MOVE_CLIMB_HOLE)) { setCurrentMove(MOVE_CLIMB_HOLE); nextMove = MOVE_NONE; }
        else { delayBusy = DELAY_RETRY_LATER; setCurrentMove(MOVE_NONE); nextMove = MOVE_CLIMB_HOLE; }
      }
      // - he has escaped the trap (make a normal AI move)
      else if (currentMove == MOVE_CLIMB_HOLE) { nextMove = MOVE_NONE; super.makeNextMove(); isTrapped = false; }
    }
    // b/ this vilain should respawn but cannot
    else if (currentMove == MOVE_RESPAWN && stage.isVilainAt(xTile, yTile)) delayBusy = DELAY_RETRY_LATER;
    // c/ otherwise, make a normal AI move
    else super.makeNextMove();
    ...
    if (!isTrapped)
    {
      ...
      { isTrapped = true; nextMove = MOVE_NONE; }
  ...
  protected void kill()
  {
    ...
    currentMove = MOVE_RESPAWN; delayBusy = DELAY_RESPAWN;
  }
  ...
  private void drawDiamond(Graphics g, int x, int y, int size)
  {
    ...
    if (size > 0)
    {
      g.drawLine(x - 1, y, x, y - 1); g.drawLine(x, y - 1, x + 1, y);
      g.drawLine(x + 1, y, x, y + 1); g.drawLine(x, y + 1, x - 1, y);
      if (size > 1)
      {
        g.drawLine(x - 2, y, x, y - 2); g.drawLine(x, y - 2, x + 2, y);
        g.drawLine(x + 2, y, x, y + 2); g.drawLine(x, y + 2, x - 2, y);
      }
  ...
  public boolean paint(Graphics g)
  {
    ...
    if (!isVisible)
    {
      ...
      if (Math.abs(x) > cw) z = Math.min(z, Math.abs(x) * ch);
      if (Math.abs(y) > ch) z = Math.min(z, Math.abs(y) * cw);
      x = (cx + cw * ch * x / z) / 2;
      y = (cy + cw * ch * y / z) / 2;
      // Signal this vilain's direction with a small diamond
      int size = 0;
      if (100 * z < 200 * cw * ch) size++;
      if (100 * z < 130 * cw * ch) size++;
