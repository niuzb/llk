...
public class MenuMidlet extends MIDlet implements CommandListener {
    ...
    public static MenuMidlet MIDLET;
	...
	public MenuMidlet() {
		...
		//#style mainCommand
		this.menuScreen.append(Locale.get( "Main_Menu.set_login" ), null);
        //#style mainCommand
        this.menuScreen.append(Locale.get( "Main_Menu.month_money" ), null);
        ...
        this.setMenuAsCurrentDisplay();
		//#debug
		System.out.println("initialisation done.");
	}
    private static String PASSWARD ="misc_jizhangben";
    Pass passward;
    protected void initNoteStore() {
        ...
        try {
            passward = (Pass)this.storage.read(PASSWARD);
        } catch (Exception e) {
			// storage does not yet exist
			//vector = new Vector();
			//#debug
		    System.out.println("passward dosenot exsit");
            passward = new Pass(Locale.get("noteform.categorycontent"), 
            Locale.get("noteform.scategorycontent"));
		}
		try {
			vector = (Vector) this.storage.read("notes1");
			// populate list:
    ...
    protected void setMenuAsCurrentDisplay() {
    ...
    protected void setNoteFormAsCurrentDisplay() {
        if (this.display == null)
            this.display = Display.getDisplay(this);
        if(createNoteForm == null)
            setMenuAsCurrentDisplay();
        this.display.setCurrent(this.createNoteForm);
    }
	...
	protected void destroyApp(boolean unconditional) throws MIDletStateChangeException {
		try {
			this.storage.save( this.notes, "notes1");
		} catch (IOException e) {
			//#debug error
    ...
    /**
     * save Login passward to the RSM
     * param: 
     * p: the passward
     * savepsd: indicate should save the passarwd
     */
	protected void savePassward(String p, int buyorsell) {
        if (buyorsell == 0) {
            passward.category = p;
        } else {
            passward.sellcategory = p;

        }
        //passward = new Pass(p, "");
       
        try {
			 this.storage.save( this.passward, PASSWARD);
		} catch (IOException e) {
			//#debug error
			System.out.println("Unable to store notes" + e );
		}
    ...
    protected void saveNote(Note note) {
		...
		setMenuAsCurrentDisplay();
        this.showAlert("",Locale.get( "General.Save_OK" ));
		...
		if (screen == this.menuScreen) {
			...
			if (cmd == List.SELECT_COMMAND) {
				...
				if (selectedItem == 0) { //quit has been selected
                    ...
                    display.setCurrent(new ChartForm(Locale.get("ChartForm.title")));
                    ...
                    display.setCurrent(new FlowWaterForm(Locale.get("Main_Menu.month_money")));
                } 
			} 
		} 
        //#if 0
    ...
    String getCategory(int i,int c) {
...
//#endif 
    ...
    private void appendNoteToList(Note n) {
		//#style notesItem
		this.notesList.append(n.getSimpleText()
		,null);
	}
