...
class LodeRunnerHero extends LodeRunnerCharacter
{
  ...
  public void moveToTile(int tileIndex)
  {
    ...
    delayMessage = 0; currentMessage = null;
  }
  ...
  public int getFrame()
  {
    ...
    if (delayBusy > 2)
    {
      if (currentMove == MOVE_DIG_RIGHT) return 72;
      else if (currentMove == MOVE_DIG_LEFT) return 73;
    }
  ...
  protected void setCurrentMove(int move)
  {
    ...
    {
      case MOVE_DIG_LEFT:
        lookLeft = true; xDelta = 0; yDelta = 0;
        delayBusy = 6; // Hero will be busy during 6 heartBeats
        break;
      case MOVE_DIG_RIGHT:
        lookLeft = false; xDelta = 0; yDelta = 0;
        delayBusy = 6; // Hero will be busy during 6 heartBeats
        break;
    }
  }
  ...
  public void requestMove(int move)
  {
    ...
    if (move == getReverseMove()) setCurrentMove(move);
  }
  ...
  protected boolean takeChest()
  {
    ...
    if (chestTaken)
    {
      ...
      sayMessage(Integer.toString(stage.nChests-nChests)+" left" );
      GameMIDlet.MIDLET.musicManager.start_fire_music();
      if (nChests == stage.nChests) stage.enableExit();
    }
    return chestTaken;
  }
  ...
  {
    LodeRunnerCanvas cav = (LodeRunnerCanvas)GameMIDlet.MIDLET.canvas;
    if(cav != null &&
        GameMIDlet.MIDLET.rsm.autowalk == Rsm.OFF) {
        if(nextMove == MOVE_RUN_LEFT || nextMove == MOVE_RUN_RIGHT) {
            if(cav.alien_xvel == 0) 
                nextMove = MOVE_NONE;
        }
        if(nextMove == MOVE_CLIMB_DOWN || nextMove == MOVE_CLIMB_UP) {
            if(cav.alien_yvel == 0) 
                nextMove = MOVE_NONE;
        }
    }
    // Check if this hero has competed the current stage
    if (stage.exitEnabled && yTile == 0) stage.endCompleted = true;
    // Make a standard move
    super.makeNextMove();
    // If digging, mark the attempt in the stage tiles and stop moving after digging
    if (digHole(false)) nextMove = MOVE_NONE;
  }
  ...
  protected boolean isPossibleMove(int move)
  {
    ...
    else return super.isPossibleMove(move);
  }
  ...
  public boolean paint(Graphics g)
  {
    ...
    if (currentMove == MOVE_DIG_LEFT || currentMove == MOVE_DIG_RIGHT)
    {
      ...
      switch (delayBusy)
      {
        case 6: frameBlaster = lookLeft ? 80 : 78; frameMelting = 84; break;
        case 5: frameBlaster = lookLeft ? 81 : 79; frameMelting = 85; break;
        case 4: frameBlaster = 82; frameMelting = 86; break;
        case 3: frameBlaster = 83; frameMelting = 87; break;
        case 2: frameMelting = 88; break;
        case 1: frameMelting = 89; break;
      }
      int xFire = xTile + (lookLeft ? -1 : +1);
      if (frameBlaster != 0) stage.sprites[stage.spriteSize].paint(g, frameBlaster, xFire * LodeRunnerStage.SPRITE_WIDTH[stage.spriteSize], yTile * LodeRunnerStage.SPRITE_HEIGHT[stage.spriteSize]);
      if (frameMelting != 0) stage.sprites[stage.spriteSize].paint(g, frameMelting, xFire * LodeRunnerStage.SPRITE_WIDTH[stage.spriteSize], (yTile + 1) * LodeRunnerStage.SPRITE_HEIGHT[stage.spriteSize]);
    }
    // paint hero itself
    boolean isVisible = super.paint(g);
    // Render a floating message, if any
    if (delayMessage > 0 && stage.spriteSize == LodeRunnerStage.SPRITE_NORMAL) paintMessage(g);
    return isVisible;
  }
}
