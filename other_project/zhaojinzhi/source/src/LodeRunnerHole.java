...
class LodeRunnerHole
{
  ...
  {
    this.stage = stage; delayBusy = DELAY_REFILL;
    this.xTile = xTile; this.yTile = yTile;
  }
  ...
  {
    int frameHole = 0;
    if (delayBusy < DELAY_VISIBLE_REFILL) frameHole = 75;
    if (delayBusy < 2 * DELAY_VISIBLE_REFILL) frameHole = 74;
    if (frameHole == 0) return false;
