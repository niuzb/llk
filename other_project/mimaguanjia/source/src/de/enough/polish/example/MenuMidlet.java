...
public class MenuMidlet extends MIDlet implements CommandListener {
	...
	private final Command deleteCommand = new Command( Locale.get("General.Delete"), 
	Command.ITEM, 4);
	...
	public MenuMidlet() {
		...
		initNoteStore();
        firstform = new FirstForm(Locale.get( "messages.welcome" ));
        this.display = Display.getDisplay(this);
		this.display.setCurrent(firstform);
		//#debug
		System.out.println("initialisation done.");
	}
    private FirstForm firstform;
    private static String PASSWARD ="passward";
    Pass passward;
     ...
     */
	protected void savePassward(String p, boolean savepsd) {
       
        passward = new Pass(p, savepsd);
		...
		if (screen == this.menuScreen) {
			...
			if (cmd == List.SELECT_COMMAND) {
				...
				if (selectedItem == 0) { //quit has been selected
                    ...
                    display.setCurrent(new SetForm(Locale.get("Main_Menu.set_login" )));
                } 
			} 
		} 
